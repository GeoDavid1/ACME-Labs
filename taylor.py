...
def interiorPoint(A, b, c, niter=20, tol=1e-16, verbose=False):
    ...
    def _search_direction(x, l, mu, s=0.1):
        ...
        v = np.dot(x, mu) / n       # function given right above equation (16.2)
        b = -F(x, l, mu) + np.hstack((np.zeros(m+n), v * s * np.ones(n)))
        return la.lu_solve(la.lu_factor(DF), b)   # solve DF * d = b
    
    def _step_length(x, l, mu):

        d = _search_direction(x, l, mu)                 # search direction

        d_mu, d_x = d[-n:], d[:n]                       # split d into d_mu and d_x
        mu_mask, x_mask = d_mu < 0, d_x < 0             # create masks for negative values

        alpha_max = np.min(-mu[mu_mask] / d_mu[mu_mask]) if (d_mu < 0).any() else 1      # calculate alpha_max
        d_max = np.min(-x[x_mask] / d_x[x_mask]) if (d_x < 0).any() else 1                  # calculate d_max
        alpha_max = np.min((-mu/d_mu)[d_mu < 0])                                        # calculate alpha_max   
        d_max = np.min((-x/d_x)[d_x < 0])                                               # calculate d_max
        return min(1, 0.95 * alpha_max), min(1, 0.95 * d_max), d                        # return alpha, delta, and d
    
    def solve():

        m, n = A.shape                                              # get dimensions of A
        x, l, mu = starting_point(A, b, c)                          # get starting point

        for _ in range(niter):                                      # iterate niter times

            alpha, delta, solution = _step_length(x, l, mu)         # get alpha, delta, and d
            
            x = x + delta * solution[:n]                            # update x, l, and mu
            l = l + alpha * solution[n:-n]
            mu = mu + alpha * solution[-n:]

            if np.dot(x, mu)/n < tol:                               # check for convergence
                break
...
def leastAbsoluteDeviations(filename='simdata.txt'):
    ...
    solution = interiorPoint(A, y, c)[0]
    beta = solution[m:m+n] - solution[m+n:m+2*n]
    b = solution[m+2*n] - solution[m+2*n+1]
