...
df_anthro$intake_total_energy_kcal <- df_anthro$intake_energy_kj * 0.239006
df_anthro$intake_total_energy_kcal_std <- scale(log(df_anthro$intake_total_energy_kcal))

df_blood <- read.csv("../data/processed_files/subsetted_df_blood.csv")
df_blood$intake_total_energy_kcal <- df_blood$intake_energy_kj * 0.239006
df_blood$intake_total_energy_kcal_std <- scale(log(df_blood$intake_total_energy_kcal))

# Define change amounts
change <- 80  # kcal; 20 g = 80 kcal
change_fiber <- 10  # kcal ; 5 g = 10 kcal
...
calculate_change <- function(model, predictor, substitution, change, change_sub) {
  ...
  posterior_samples <- extract.samples(model, n = 10000)
  
  # Access the samples for the coefficients
  beta_predictor_samples <- posterior_samples$b_predictor
  beta_substitute_samples <- posterior_samples$b_substitute
...
quap_model_anthro <- function(predictor_var, substitute_var, energy_var, outcome_var, data) {
  quap(
    ...
    data = data.frame(
      ...
      energy = data[['intake_total_energy_kcal_std']],
...
quap_model_anthro_fiber <- function(predictor_var, substitute_var, energy_var, outcome_var, data) {
  quap(
    alist(
      outcome ~ dnorm(mu, sigma),
      mu <- a_gender[gender] + 
...
# Define quap_model function
quap_model_blood_fiber <- function(predictor_var, substitute_var, energy_var, outcome_var, data) {
  quap(
    alist(
      outcome ~ dnorm(mu, sigma),
      mu <- a_gender[gender] + 
...
define_models_anthro <- function(outcome) {
  list(
    ...
    m_starch_fiber = quap_model_anthro_fiber(
    ...
    m_intrinsic_sugar_fiber = quap_model_anthro_fiber(
      predictor_var = "intake_intrinsic_sugars_kcal_std",
      substitute_var = "intake_fibre_englyst_kcal_std",
    ...
    m_nmes_fiber = quap_model_anthro_fiber(
      predictor_var = "intake_nmes_kcal_std",
      substitute_var = "intake_fibre_englyst_kcal_std",
...
define_models_blood <- function(outcome) {
  list(
    ...
    m_starch_fiber = quap_model_blood_fiber(
      predictor_var = "intake_starch_kcal_std",
      substitute_var = "intake_fibre_englyst_kcal_std",
      energy_var = "intake_energy_kcal_fiber_starch",
      outcome_var = outcome,
    ...
    m_intrinsic_sugar_fiber = quap_model_blood_fiber(
      predictor_var = "intake_intrinsic_sugars_kcal_std",
    ...
    m_nmes_fiber = quap_model_blood_fiber(
      predictor_var = "intake_nmes_kcal_std",
      substitute_var = "intake_fibre_englyst_kcal_std",
      energy_var = "intake_energy_kcal_fiber_nmes",
      outcome_var = outcome,
...
output_file <- "../results/substitutions/bodyfat_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_bf, variable_mapping, df_anthro, change, change_fiber, output_file)

models_trig <- define_models_blood("trig_log")

# Specify output file path
output_file <- "../results/substitutions/trig_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_trig, variable_mapping, df_blood, change, change_fiber, output_file, trig = TRUE)

models_hdl <- define_models_blood("hdl")

# Specify output file path
output_file <- "../results/substitutions/hdl_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_hdl, variable_mapping, df_blood, change, change_fiber, output_file)

models_ldl <- define_models_blood("ldl")

# Specify output file path
output_file <- "../results/substitutions/ldl_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_ldl, variable_mapping, df_blood, change, change_fiber, output_file)

models_hba1c <- define_models_blood("hba1c_percent")

# Specify output file path
output_file <- "../results/substitutions/hba1c_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_hba1c, variable_mapping, df_blood, change, change_fiber, output_file)

models_bmi <- define_models_anthro("bmi")

# Specify output file path
output_file <- "../results/substitutions/bmi_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_bmi, variable_mapping, df_anthro, change, change_fiber, output_file)

models_wc <- define_models_anthro("waistcirumference")

# Specify output file path
output_file <- "../results/substitutions/waistcirumference_substitution_results.csv"

# Call the function to iterate over models, calculate results, and generate outputs
iterate_and_calculate_results(models_wc, variable_mapping, df_anthro, change, change_fiber, output_file)